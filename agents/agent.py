...
class StudlyAgent:
    ...
    async def process_messages_async(
        ...
        """Process incoming messages asynchronously and return a personalized study plan."""
        ...
        # Retrieve past context with lock protection
        async with self._context_lock:
            history = self.study_contexts.get(context_id, [])
        
        study_plan = await self._generate_study_plan_async(user_text)
        
        # Cache this interaction with lock protection
        async with self._context_lock:
            self.study_contexts[context_id] = history + [user_text]
    ...
    def _generate_study_plan(self, query: str) -> str:
        """Call Gemini model via LangChain synchronously."""
        try:
    ...
    async def _generate_study_plan_async(self, query: str) -> str:
        ...
        try:
            ...
            print("Gemini timeout: Request took too long")
            return "The study plan is taking longer than expected. Please try again with a simpler request."
        except Exception as e:
            print(f"Gemini error: {e}")
            return "I encountered an issue generating the study plan. Please try again."
